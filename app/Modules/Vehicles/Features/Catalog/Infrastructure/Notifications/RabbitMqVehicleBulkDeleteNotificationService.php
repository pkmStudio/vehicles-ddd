...
/**
 * Публикует результат массового удаления автомобилей во внешний RabbitMQ-транспорт.
 */
final readonly class RabbitMqVehicleBulkDeleteNotificationService implements VehicleBulkDeleteNotificationServiceInterface
{
    ...
    /**
     * Публикует результат bulk-delete автомобилей наружу.
     */
    public function notify(VehicleBulkDeleteResultDTO $result): void
    {
        $this->publisher->publish(new RabbitMessageDTO(
            ...
            data: new VehiclesCatalogBulkDeleteCompleted(
                ...
                errors: array_map(
                    static fn (VehicleBulkDeleteErrorDTO $error): VehiclesCatalogBulkDeleteError => new VehiclesCatalogBulkDeleteError(
                        id: $error->id,
                        reason: $error->reason,

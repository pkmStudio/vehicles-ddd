...
| `app/Vehicles/Application/Import/UseCases/Engine/AssignEngineGroupUseCase.php:23` | метод `execute()` после `setGroupId()` | `EngineGroupAssigned`, `EngineGroupReassigned` | Для downstream-реакций на изменение связки кода двигателя с группой. |
| `app/Vehicles/Application/Import/UseCases/EngineModification/LinkEngineModificationFromRowUseCase.php:27` | метод `execute()` после `syncWithoutDetaching()` | `EngineModificationLinkCreated` | Реакции на установку связи `engine <-> modification` (кэш, индексация, отчёты). |
| `app/Vehicles/Application/Import/UseCases/Vehicle/UpsertVehicleWiperSpecUseCase.php:34` | метод `execute()` после батчевого апсерта | `VehicleWiperSpecsUpdated` | Событие факта массового обновления дворников. |
| `app/Vehicles/Application/Import/UseCases/Engine/UpsertSparkPlugSpecByModificationUseCase.php:33` | метод `execute()` после цикла по двигателям | `EngineSparkPlugSpecsUpdated` | Факт пакетного обновления свечей по модификации (включая skipped-результат). |
| `app/Vehicles/Application/Import/UseCases/Engine/UpsertEngineSparkPlugSpecUseCase.php:31` | метод `execute()` после upsert | `EngineSparkPlugSpecUpdated` | Мелкий факт обновления свечей конкретного двигателя. |
| `app/Vehicles/Application/Import/UseCases/Reporting/ReportImportResultUseCase.php:25` | метод `execute()` после `reporter->store()` / catch | `ImportReportGenerated`, `ImportReportFailed` | Убрать `TODO`-развязку и формализовать итог импорта. |

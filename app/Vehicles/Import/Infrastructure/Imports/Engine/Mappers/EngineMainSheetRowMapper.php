...
namespace App\Vehicles\Import\Infrastructure\Imports\Engine\Mappers;
...
final readonly class EngineMainSheetRowMapper
{
    ...
    public function map(array $row): EngineSheetRowDTO
    {
        return new EngineSheetRowDTO(
            ...
            engineCapacity: $this->formatter->nullableString($row[2] ?? null),
            cylinderDiameter: $this->formatter->nullableFloat($row[7] ?? null, 'cylinder_diameter'),
            cylinderCount: $this->formatter->nullableInt($row[6] ?? null, 'cylinder_count'),
            engNumberOfValves: $this->formatter->nullableInt($row[8] ?? null, 'eng_number_of_valves'),
            engFuelType: null,
        );
    }
}

...
унаследованные странности dan-center: `brands.date_begin` переименован в `brands.date_end` для
даты окончания действия сертификата, а `nomenclatures.weight` нормализован в положительный integer
в граммах).
...
1. **`KitGrouping`** — авто-группировка остатков номенклатуры в кандидаты на новые наборы. Не
...
2. **Мелкая доработка робастности `Import`**: все 3 Excel-адаптера (`Nomenclature`/`PackDimension`/
...
3. **`rabbit-transport:setup` не запускался** — RabbitMQ publish (уведомления о завершении
...
> Доменные события `NomenclatureUpdated`/`KitUpdated` для MpCard-инвалидации перенесены в
> `plan-new.md` — это кросс-доменная задача (карточки зависят и от машин/Vehicles, и от
> номенклатуры/Warehouse), не специфичная для этого документа.
...
- **MoySklad-синхронизация** — детальный план (пакет `pkmstudio/moysklad-client` + фича
  `Warehouse/MoySklad`) расписан отдельно в `plan-moysklad.md`, реализация не начата.
- **MpCard-инвалидация** (сам транспорт/интеграция с CRM, не только события из п.2) — будущая
  задача, вне текущего плана.

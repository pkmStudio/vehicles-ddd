...
/**
 * Проверяет правила обязательности details-шаблона номенклатуры `cabinFilter`.
 */
final class NomenclatureCabinFilterDetailsDataFactoryTest extends TestCase
{
    public function test_cabin_filter_details_require_all_declared_fields_and_one_value_per_metric(): void
    {
        $index = 0;

        $details = app(NomenclatureDetailsDataFactory::class)->make(
            template: NomenclatureDetailTemplateEnum::CABIN_FILTER,
            row: self::validRow(),
            index: $index,
        );

        self::assertInstanceOf(CabinFilterDetailsData::class, $details);
        self::assertSame('LONG_TERM', $details->performance);
        self::assertSame('RECTANGLE', $details->form);
        self::assertFalse($details->frame);
        self::assertSame('CARBON', $details->filterType);
    ...
    public function test_cabin_filter_details_require_every_field(int $emptyIndex, string $field): void
    {
        $index = 0;
        $row = self::validRow();
        $row[$emptyIndex] = null;

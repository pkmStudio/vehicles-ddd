...
{
    private function service(PackagingServiceInterface $packaging, KitComplectationServiceInterface $complectation, ?TypeTemplateResolverInterface $wiperResolver = null): KitPropertiesService
    {
    ...
    public function test_builds_properties_for_single_type_kit(): void
    {
        ...
        $packaging->shouldReceive('selectOrCreate')
            ->once()
            ->with(
                Mockery::on(fn (PackagingTypeData $t): bool => $t->id === 1 && $t->name === 'Колодки'),
                Mockery::on(fn (array $noms): bool => count($noms) === 2 && $noms[0] instanceof PackagingNomenclatureData),
            )
            ->andReturn($box);
    ...
    public function test_excludes_adapter_from_primary_but_includes_its_weight(): void
    {
        ...
        $box = new PackagingPackDimensionData(name: 'Box', weight: 10, width: 10, height: 10, length: 10, price: 5, typeId: 3, id: 5);
        ...
        $packaging->shouldReceive('selectOrCreate')
            ...
            ->once()
            ->with(
                Mockery::on(fn (PackagingTypeData $t): bool => $t->id === 3),
